    ...
    public class VA012_ENBD
    {
        ...
        public StatementResponse ImportStatement(Ctx ctx, string FileName, string _path, int _bankaccount, int _bankAccountCurrency, string _statementno, string _statementCharges)
        {
            ...
            try
            {
                ...
                {
            new SqlParameter("@statementno", _statementno)
                };

                _ds = DB.ExecuteDataset("SELECT C_BANKSTATEMENT_ID,DOCSTATUS FROM C_BANKSTATEMENT WHERE ISACTIVE='Y' AND NAME=@statementno AND STATEMENTDATE BETWEEN " + GlobalVariable.TO_DATE(_startdate, true) + " AND " + GlobalVariable.TO_DATE(_enddate, true), param1);
                        ...
                        if (ds != null)
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                if (dt.Columns[0].ColumnName == "F1" && Convert.ToString(dt.Rows[0][0]) == "Account Statement")
                                {
                                    ...
                                    {
                                new SqlParameter("@currency", accountCurrency)
                                    };

                                    _C_Currency_ID = Convert.ToInt32(DB.ExecuteScalar("SELECT C_CURRENCY_ID FROM C_CURRENCY WHERE LOWER(DESCRIPTION) LIKE LOWER(@currency) OR LOWER(ISO_CODE) LIKE LOWER(@currency)", param2,null));
                                    ...
                                    for (int i = -1; i < dt.Rows.Count; i++)
                                    {
                                        ...
                                        else
                                        {
                                            if ((Convert.ToString(dt.Rows[i][0]) != string.Empty) && (Convert.ToString(dt.Rows[i][1]) != string.Empty))
                                            {
                                                ...
                                                _BnkStmtLine.SetLine(lineno);
                                                lineno = lineno + 10;
                                                _BnkStmtLine.SetStatementLineDate(DateTime.Parse(dt.Rows[i][1].ToString()));
                                                _BnkStmtLine.SetDateAcct(DateTime.Parse(dt.Rows[i][1].ToString()));
                                                _BnkStmtLine.SetValutaDate(DateTime.Parse(dt.Rows[i][1].ToString()));
                                                _BnkStmtLine.SetDescription(Convert.ToString(dt.Rows[i][2]));
